          ...
          {company.highlights && company.highlights.length > 0 && (
            <section className="bg-white/5 border border-white/10 rounded-xl p-5">
              ...
              <div className="flex flex-wrap gap-2.5">
                {company.highlights.map((hl, idx) => (
                  <span key={idx} className="px-3 py-1.5 bg-[#000000] text-white text-sm font-semibold rounded-md border border-white/20 shadow-sm">
                    {hl}
                  </span>
                ))}
              </div>
